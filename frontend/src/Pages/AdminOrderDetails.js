...
const OPERATIONAL_STATUSES = ["placed", "confirmed", "preparing", "ready", "out_for_delivery", "cancelled"];
const NEXT_STATUSES = {
...
const getNextStatuses = (order) => {
  ...
  if (order.status === "ready" && order.deliveryType === "delivery") next.unshift("out_for_delivery");
  return next;
};
  ...
  return (
    <main className="admin-order-details-page">
      ...
      <section className="admin-order-details-card admin-order-status-card">
        <div><h2>Order Status</h2><p>{completed ? "This order is completed through handover verification." : "Follow the restaurant workflow. Completed delivery or pickup is recorded through handover verification."}</p></div>
        <select value={order.status || ""} disabled={completed || saving || nextStatuses.length === 0} onChange={(event) => updateStatus(event.target.value)} aria-label="Update order status">
          {completed ? <option value={order.status}>{label(order.status)}</option> : <><option value={order.status}>{label(order.status)} (current)</option>{nextStatuses.map((status) => <option key={status} value={status}>{label(status)}</option>)}</>}
        </select>
      </section>
