...
const AdminMenu = () => {
  ...
  const handleSubmit = (event) => {
    ...
    const nextFoods = editingFood
      ? foods.map((food) => (food.id === editingFood.id ? payload : food))
      : [payload, ...foods];
  ...
  const toggleAvailability = (food) => {
    persist(foods.map((item) => item.id === food.id ? { ...item, isAvailable: !item.isAvailable } : item));
  };
    ...
    <section className="admin-menu-page">
      <div className="admin-menu-header">
        <div>
          ...
          <h2>Food & Menu</h2>
          <p>Manage dishes, pricing and availability from one place.</p>
        </div>
      ...
      <div className="admin-menu-table-card">
        <div className="admin-menu-table-heading"><div><span>Menu inventory</span><h3>{filteredFoods.length} item{filteredFoods.length !== 1 ? "s" : ""}</h3></div><button type="button" onClick={() => navigate("/menu")}>View Customer Menu →</button></div>
        <div className="admin-menu-table-wrap">
            ...
            <tbody>
              {filteredFoods.map((food) => (
                <tr key={food.id}>
                  ...
                  <td><strong className="admin-menu-price">${Number(food.price).toFixed(2)}</strong></td>
          ...
          {!filteredFoods.length && <div className="admin-menu-empty"><span>🍽️</span><strong>No food found</strong><p>Try another search or filter.</p></div>}
        </div>
      </div>

      {editingFood !== null || form.name !== "" || form.price !== "" ? (
        <div className="admin-menu-modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && closeModal()}>
          <form className="admin-menu-modal" onSubmit={handleSubmit}>
            ...
            <div className="admin-menu-form-grid">
              ...
              <label className="wide">Image URL<input value={form.image} onChange={(event) => setForm({ ...form, image: event.target.value })} placeholder="Optional image URL" /></label>
              <label className="wide">Description<textarea rows="3" value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></label>
              <label className="wide">Ingredients<input value={form.ingredients} onChange={(event) => setForm({ ...form, ingredients: event.target.value })} placeholder="Tomato sauce, cheese, vegetables" /></label>
            ...
            <div className="admin-menu-modal-actions"><button type="button" className="cancel" onClick={closeModal}>Cancel</button><button type="submit">{editingFood ? "Save Changes" : "Add Food"}</button></div>
          </form>
        </div>
      ) : null}
    </section>
